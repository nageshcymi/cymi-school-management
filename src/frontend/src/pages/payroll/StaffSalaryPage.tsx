...
import { STAFF_SALARY_DATA, type StaffSalary } from "../../data/accountsData";
...
export default function StaffSalaryPage() {
  ...
  const [data, setData] = useState<StaffSalary[]>(STAFF_SALARY_DATA);
  ...
  const filtered = useMemo(
    () =>
      data.filter((e) => {
        ...
        const matchSt = status === "all" || e.status.toLowerCase() === status;
        return matchS && matchD && matchSt;
      }),
    [data, search, dept, status],
  );
  ...
  function openAdd() {
    setDialog({
      open: true,
      mode: "add",
      item: { status: "Active", department: "Mathematics" },
    });
  }
  function openEdit(item: StaffSalary) {
  ...
  function handleSave() {
    ...
    if (dialog.mode === "add") {
      ...
      setData((d) => [...d, { ...item, id: newId } as StaffSalary]);
      toast.success("Staff salary record added");
    } else {
      setData((d) =>
        d.map((e) =>
          e.id === item.id ? ({ ...e, ...item } as StaffSalary) : e,
        ),
      );
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Staff Salary</h1>
            <p className="text-gray-500 text-sm mt-1">
              Salary structure for all staff members
            </p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {paginated.map((e, i) => (
                  ...
                  >
                    <td className="px-3 py-2.5 text-xs font-mono text-blue-600">
                      {e.empId}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-gray-600">{fmt(e.hra)}</td>
                    <td className="px-3 py-2.5 text-gray-600">{fmt(e.da)}</td>
                    <td className="px-3 py-2.5 text-red-600">
                      {fmt(e.pfDeduction)}
                    </td>
                    <td className="px-3 py-2.5 text-red-600">{fmt(e.tds)}</td>
                    ...
                    <td className="px-3 py-2.5">
                      <Badge
                        ...
                        className={`text-xs ${e.status === "Active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                      >
                        {e.status}
                      </Badge>
                    </td>
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {dialog.mode === "add" ? "Add" : "Edit"} Staff Salary
            </DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-3 py-2">
            {[
              ...
              { label: "HRA (\u20b9)", field: "hra" },
              { label: "DA (\u20b9)", field: "da" },
              { label: "Other Allow (\u20b9)", field: "otherAllowances" },
              { label: "PF Deduction (\u20b9)", field: "pfDeduction" },
              { label: "TDS (\u20b9)", field: "tds" },
              { label: "Net Pay (\u20b9)", field: "netPay" },
            ].map(({ label, field }) => (
              <div key={field}>
